...
func TestHandler_RefreshAccessToken(t *testing.T) {
	...
	user, _ := models.NewUser("andrew", email, pass)
	userRepo.Save(user)
...
func TestHandler_RefreshExpired(t *testing.T) {
	...
	expRefreshToken := &models.RefreshToken{
		ID:        "",
		UserID:    user.ID,
		Token:     "",
		CreatedAt: time.Now(),
	...
	expToken, err := models.CreateToken(expRefreshToken)
	if err != nil {
...
func TestHandler_Login(t *testing.T) {
	...
	pass := "12345678"
	user, _ := models.NewUser("andrew", "andrewhoj@gmail.com", pass)
	userRepo.Save(user)
